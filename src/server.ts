...
/**
 * Shared by validateConfig() and createServer() so the two can never quietly diverge on what
 * "the configured cipher" means — validateConfig's whole point is mirroring what createServer
 * actually runs at connection time.
 */
function cipherFor(config: Partial<Config>): Cipher | undefined {
    return config.encryptionKeyHex ? Cipher.fromHex(config.encryptionKeyHex) : undefined;
}
...
export function validateConfig(db: Database.Database, config: Partial<Config> = {}): void {
    new CacheStore(db, config, cipherFor(config));
}
